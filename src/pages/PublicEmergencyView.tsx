...
export default function PublicEmergencyView() {
  ...
  const { data: conditions = [] } = useConditions(userId);
  const { data: implants = [] } = useImplants(userId);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            {language === 'de' 
              ? 'Dieses Notfall-Profil existiert nicht oder wurde gelöscht.'
              : 'This emergency profile does not exist or has been deleted.'}
          </p>
        </div>
      </div>
    );
